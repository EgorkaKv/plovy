...
class FaceMeshCameraImageAdapter {
  ...
  static FaceMeshNv21Image? _fromSinglePlaneNv21(CameraImage image) {
    ...
    final int ySize = rowStride * image.height;
    final int vuSize = rowStride * (image.height ~/ 2);
  ...
  static FaceMeshNv21Image? _fromYAndInterleavedVuPlanes(CameraImage image) {
    final Uint8List? yPlane = _copyPlane(
      image.planes[0],
      width: image.width,
      height: image.height,
    );
    final Uint8List? vuPlane = _copyPlane(
      image.planes[1],
      width: image.width,
  ...
  static FaceMeshNv21Image? _fromYuv420Planes(CameraImage image) {
    ...
    final Uint8List vuPlane = Uint8List(image.width * uvHeight);
    ...
    for (int row = 0; row < uvHeight; row++) {
      for (int col = 0; col < uvWidth; col++) {
        ...
        final int outIndex = row * image.width + col * 2;
        vuPlane[outIndex] = v;
        vuPlane[outIndex + 1] = u;
      }
    }
